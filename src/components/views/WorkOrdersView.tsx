...
import { 
  ...
  Car, Trash2, Edit, Save, PlusCircle
} from 'lucide-react';
...
export const WorkOrdersView: React.FC<WorkOrdersViewProps> = ({
  ...
  // Pipeline Steps (Matching Image #4)
  const pipelineSteps = [
    { key: 'pending', number: 1, label: '1. الاستلام' },
    { key: 'inspection', number: 2, label: '2. الفحص والأعطال' },
    { key: 'in_progress', number: 3, label: '3. جاري الصيانة' },
    { key: 'ready', number: 4, label: '4. جاهز للتسليم' },
    { key: 'delivered', number: 5, label: '5. تم الفوترة والتسليم' },
  ];
    ...
    return (
      <div className="space-y-6 pb-16">
        ...
        <div className="bg-[#0F172A] border border-slate-800 rounded-3xl p-6 space-y-6">
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <span className="px-3 py-1 rounded-full text-xs font-bold bg-orange-500/20 text-orange-400 border border-orange-500/30">
                  {selectedOrder.status === 'in_progress' ? 'جاري العمل' : selectedOrder.status === 'ready' ? 'جاهز للتسليم' : 'نشط'}
                </span>
              </div>
          ...
          {/* Interactive 5-Step Pipeline Timeline (Matching Image #4) */}
                ...
                return (
                  ...
                  >
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${
                      isCurrent ? 'bg-orange-500 text-white' : isCompleted ? 'bg-emerald-500 text-white' : 'bg-slate-800 text-slate-400'
                    }`}>
                      {step.number}
                    </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <button
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-bold px-4 py-2.5 rounded-2xl cursor-pointer transition-all"
              >
              ...
              <button
                onClick={() => onSelectWorkOrder(null)}
                className="flex-1 sm:flex-none bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs px-5 py-2.5 rounded-2xl cursor-pointer transition-all"
              >
                حفظ الكارت
              </button>
            </div>
            ...
            <div className="flex items-center justify-between border-b border-slate-800/80 pb-3">
              ...
              <div className="flex items-center gap-2">
                <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${
                  ...
                  {wo.status === 'in_progress' ? 'في العمل' : wo.status === 'ready' ? 'جاهز للتسليم' : wo.status === 'waiting_parts' ? 'بانتظار قطع' : 'مكتمل'}
                </span>
                <button
